...
switch($basis){
	case 'ordercount':
	$query_part = 'COUNT(DISTINCT orders.order_id)';
	break;
	case 'sales':
...
$query = 'SELECT products.vendor_id, '.$query_part.' AS rankval
			FROM orders, order_product, products
				WHERE '.$date_condition.' AND orders.order_status="Delivered" AND orders.order_id=order_product.order_id AND products.product_id=order_product.product_id
...
if(isset($date_condition)){
	...
	while($row=mysqli_fetch_assoc($result2)){
		extract($row);
		$counteri = $counteri + 1;
		echo '[\''.str_replace("'","\\'",get_vendorname($vendor_id)).'\','.$rankval.']';
...
?>	
<form action="report_topvendors.php" method="POST">
<table width="90%" align="center">
<tr>
	<td width="12%">Ranking based on:</td>
	<td colspan="3"><input type="radio" name="basis" value="ordercount" <?php echo ($basis=='ordercount')? 'checked="checked"':''; ?> />Order Count by Vendor
					<input type="radio" name="basis" value="sales" <?php echo ($basis=='sales')? 'checked="checked"':''; ?> />Total Sales by Vendor
	</td></tr><tr>
	<td>Time Range:</td>
...
<tr>
	<th>Vendor Name</th>
	<th><?php echo ($basis=='ordercount')? 'Number of Orders':'Total Sales'; ?></th>
</tr>
...
while($row=mysqli_fetch_assoc($result2)){
	...
	extract($row);
	echo '<td>'.get_vendorname($vendor_id).'</td>';
	echo '<td>'.$rankval.'</td>';
	echo '</tr>';
	$odd = !$odd;
}
